...
for x in range(2,20):
    ...
    if(min_01 > np.sqrt(get_mse(user_pred, ratings_test))):
        min_01 = np.sqrt(get_mse(user_pred, ratings_test))
        index_01 = x
    ...
    if(min > np.sqrt(get_mse(user_pred_k, ratings_test))):
        min_02 = np.sqrt(get_mse(user_pred_k, ratings_test))
        index_02 = x

print(index_01, min_01)
print(index_02, min_02)
