...
for i in range(1,944):
    if(len(df_train.loc[df_train['user_id'] == i]) >= 20):
        arr[i][0] = i
...
del_user = 68
rmse_arr = np.zeros((20,2))

df_only_delected_user = df_train.loc[df_train['user_id'] == del_user]       # 유저 데이터 다 지운 index values 저장
print(df_only_delected_user.item_id.values)
print(df_only_delected_user.rating.values)
...
for x in range(20):
    print('x = ', x)
    print(ratings_train[del_user-1])
...
print(rmse_arr)
np.savetxt('./Find_Distinct_Point/rmse.txt', rmse_arr, fmt = '%f', delimiter = ',', header='test')
