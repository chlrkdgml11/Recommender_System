...
def sim_item(item1,item2):
    ...
    rating1 = []
    rating2 = [] #두 영화의 평점을 저장할 리스트
    
    #두 사람이 모두 본 영화만 리스트로 저장
    for movie_id in ratings[item1]:
        if movie_id in ratings[item2]:
            rating1.append(ratings[item1][movie_id])
            rating2.append(ratings[item2][movie_id])
